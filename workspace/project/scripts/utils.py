...
from pytimeloop.timeloopfe.v4.output_parsing import OutputStats, OutputStatsList
import sys
...
def single_test(result) -> OutputStats:
    return OutputStatsList([result])


def parallel_test(delayed_calls: List[Callable], n_jobs: int = 8) -> OutputStatsList:
...
def run_mapper_parse_output(
    ...
    result.scale_computes_by(
        1
        / result.variables["ENCODED_INPUT_BITS"]
        / result.variables["ENCODED_WEIGHT_BITS"]
        / result.variables["ENCODED_OUTPUT_BITS"]
    )
    return result
...
def run_layer(
    ...
    spec = get_spec(
        macro=macro, iso=iso, layer=layer, tile=tile, chip=chip, system=system, jinja_parse_data=jinja_parse_data
    )
